...
Hlavní změny oproti variantě bez indexů: oba \code{Seq Scan} + \code{Sort (external merge Disk)} nahrazeny dvěma \code{Index Only Scan} s~\code{Heap Fetches: 0}, řazení probíhá přímo po indexu.
...
Bez indexů plánovač zvolí \code{Parallel Seq Scan} přes celou tabulku \code{issue\_event\_history} a~záznamy filtruje průběžně. Paralelizace sice snižuje elapsed time, avšak všechny záznamy jsou čteny bez ohledu na hodnotu sloupce \code{event}. Odhadované náklady jsou 15\,249 při době vykonávání 31,7\,ms.
...
Bez indexů jsou v plánu tři sekvenční průchody: \code{Seq Scan} nad \code{issue\_labels\_history},\\ \code{issue\_event\_history} a~\code{issues}. Řazení dat pro okenní funkce probíhá operací \\ \code{Sort (external merge Disk)}, protože mezivýsledky překračují limit operační paměti. Tato kombinace vede k~odhadovaným nákladům 91\,102 a~době vykonávání 653,9\,ms.
