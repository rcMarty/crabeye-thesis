...
Dalším prostorem pro optimalizaci je proces inkrementální synchronizace \PR a~issues \subsectionref{subsec:decisions:github_api}. Vzhledem k~omezením aktuálně využitého \REST \API, které při dotazování na issues vrací sdílený seznam obou entit, dochází k~nadbytečnému stahování a~následnému zahazování dat. Přechod na GraphQL \API pro tuto specifickou část synchronizace by umožnil striktně oddělené dotazování obou datových typů, čímž by se eliminovala nutnost klientské filtrace a~výrazně zefektivnilo stránkování.
