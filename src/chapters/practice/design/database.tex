...
Data o~týmech jsou sice dostupná přes Team \API \sectionref{sec:rust_team_api} jako jednorázová \JSON odpověď, avšak pro analytické dotazy je nutné je mít v~databázi. Dotazy jako „soubory upravené členy daného týmu" vyžadují \SQL \code{JOIN} tabulky \code{file\_activity} s~přiřazením přispěvatelů do týmů. Bez lokálních dat by bylo nutné při každém dotazu volat Team \API a~filtrovat výsledky v~aplikační vrstvě. Data jsou proto uložena do tabulek \code{teams} a~\code{contributors\_teams} a~obnovována při každé synchronizaci, aby odrážela aktuální složení týmů.
