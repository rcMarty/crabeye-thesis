...
\end{itemize}
Každý endpoint je definován cestou. Cesta může obsahovat statické i~dynamické části. Například endpoint pro získání issues repozitáře je \code{GET /repos/\{owner\}/\{repo\}/issues}, kde \code{owner} a~\code{repo} představují dynamické části, které musí být nahrazeny konkrétními hodnotami~\cite{github_rest_api}. Ukázka takového požadavku je uvedena ve \customref{Výpisu}{lst:github_rest_example}.
...
\end{minted}
  \caption{Ukázka odpovědi \REST \API na požadavek z~\customref{Výpisu}{lst:github_rest_example} (zkráceno).}
