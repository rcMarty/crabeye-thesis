...
Dotazy jsou v~GraphQL obvykle odesílány pomocí \HTTP metody \code{POST} na jediný endpoint. Operace, které mají být provedeny, i~požadovaná návratová data jsou definovány v~těle požadavku~\cite{github_graphql_api}. Ukázka dotazu a~odpovědi je uvedena ve výpisech~\ref{lst:graphql_request_example} a~\ref{lst:graphql_response_example}.
...
\end{minted}
  \caption{Zkrácená odpověď na GraphQL dotaz z~výpisu~\ref{lst:graphql_request_example}.}
