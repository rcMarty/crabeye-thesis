...
\section{Git}
\label{sec:git}
Git je open source distribuovaný \VCS vytvořený v~roce 2005 Linusem Torvaldsem primárně pro správu zdrojového kódu jádra operačního systému Linux. Vychází z~komerčního \VCS BitKeeper a~zakládá si na několika klíčových požadavcích~\cite{chacon2014progit}:
\begin{itemize}
  \item Rychlost
...
\begin{description}
  ...
  \item [Adresář Gitu (Git directory neboli \code{.git})] obsahuje historii a~metadata projektu.
\end{description}
...
Při jakékoliv běžné operaci v~Gitu jsou data do databáze pouze přidávána, nikdy mazána. Ke ztrátě zapsaných dat může dojít jedině při záměrném použití pokročilých operací, které mění historii repozitáře. Díky tomu lze bez obav provádět rozsáhlé experimenty nebo změny, protože k~dříve zapsanému stavu se lze vždy vrátit~\cite{chacon2014progit}.
...
Git databáze je fundamentálně založena na principu souborového systému s adresovatelným obsahem jako úložiště klíč--hodnota, kde klíčem je hash obsahu a hodnotou je samotný soubor. Tento klíč je při každém commitu vypočítáván a vrácen pro pozdější použití.
...
\begin{description}
  \item[Bloby (Binary Large Objects)] \label{par:git:bloby}
  ...
  \item[Stromy (Tree Objects)] \label{par:git:tree_objects}
  ...
  \item[Commity (Commit Objects)] \label{par:git:commit_objects}
    I~když stromový objekt zachycuje snímek projektu, neobsahuje kontext o~tom, kdo tento stav uložil, kdy a z~jakého důvodu.

    Každý commit obsahuje tato metadata:
...
V~Gitu jsou naproti tomu větve pouze pohyblivé ukazatele směřující na konkrétní commit. Fyzicky se jedná o~jeden textový soubor ve složce \code{.git/refs/heads/}, který obsahuje pouze hash commitu. Vytvoření větve tedy znamená vytvoření nového souboru s~hashem commitu, což je velice rychlé bez ohledu na velikost projektu~\cite{chacon2014progit}.
...
\label{sec:git_api}
Jeden z mnoha programovatelných přístupů ke Gitu je knihovna \devTool{libgit2}{https://libgit2.org/}. Jedná se o reimplementaci Git knihovny napsanou v~jazyce C bez jakýchkoli externích závislostí, se zaměřením na programátorsky přívětivé \API{}.
Knihovna poskytuje také tzv. language bindings pro různé jazyky, včetně Rustu, Pythonu, Ruby a~dalších. Díky tomu je možné využívat Git pohodlně v~ostatních aplikacích bez nutnosti znalosti jazyka C~\cite{chacon2014progit}.

Jedna z~nejzajímavějších vlastností této knihovny je podpora tzv. backendů (pluginů). Libgit2 umožňuje vývojářům napojit vlastní úložiště pro různé typy operací a~vytvářet vlastní pravidla pro konfiguraci, ukládání referencí nebo i~pro databázi objektů~\cite{chacon2014progit}.
