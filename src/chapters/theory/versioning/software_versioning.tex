...
\begin{figure}[ht]
  \centering
  \includegraphics[height=0.7\textwidth]{figures/vcs/Revision_controlled_project_visualization.pdf}
...
Rozdíl je patrný i~v~principu odesílání změn. U~centrálního \VCS se každá změna zapisuje přímo na server, zatímco u~distribuovaného \VCS se změna nejprve zaznamená do lokálního repozitáře a~teprve následně se propaguje na hostovací službu. Tedy u~centrálního \VCS je vyžadováno připojení k~serveru pro publikování změn, zatímco u~distribuovaného není vyžadováno a~změny na server lze zapsat později.

Rozdíl je patrný i~v~principu řešení konfliktů a~slučování větví. Centralizovaný \VCS umožňuje provést aktualizaci ze serveru, i~když má uživatel rozpracované, dosud nezapsané soubory, a~systém automaticky smíchá všechny změny, což může vést ke konfliktům. Distribuovaný \VCS uživatele nejprve donutí zaznamenat změny do lokální historie a~teprve poté lze provést sloučení mezi dvěma pevnými body historie, z~nichž vzniká nový pevný bod.
