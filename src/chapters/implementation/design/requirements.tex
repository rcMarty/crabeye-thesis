...
\begin{enumerate}
  \item \textbf{Integrace dat z více zdrojů}
    \begin{itemize}
      \item Systém musí načítat data o~\PR a~issues z~GitHub \API.
      \item Systém musí načítat informace o~změněných souborech z~historie git repozitáře.
      \item Systém musí načítat informace o~členech týmů a~kontributorech, které jsou potřebné pro analytické dotazy.
    \end{itemize}
  ...
  \item \textbf{Analytické zpracování}
    \begin{itemize}
      ...
      \item Systém musí podporovat filtrování podle repozitáře, času a dalších parametrů dotazu.
  ...
  \item \textbf{Zpřístupnění dat přes API}
    \begin{itemize}
      ...
      \item Systém musí vracet odpovídající \HTTP kódy pro běžné i~chybové stavy (např. 200, 400, 404, 500).
      \item Systém musí poskytovat strojově čitelnou dokumentaci \API (OpenAPI/Swagger).
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Výkon}
    \begin{itemize}
      \item Systém musí efektivně zpracovávat data z velkého repozitáře.
      \item Systém musí omezovat objem vracených dat pomocí stránkování a limitů.
      \item Databázová vrstva musí využívat vhodné indexy pro často používané dotazy.
    \end{itemize}
  ...
  \item \textbf{Spolehlivost a odolnost}
    \begin{itemize}
      ...
      \item Při dílčích chybách musí systém chybu zalogovat a umožnit opětovné spuštění synchronizace.
    \end{itemize}

  \item \textbf{Konzistence a kvalita dat}
    \begin{itemize}
      \item Systém musí minimalizovat duplicitní záznamy (např. pomocí upsert logiky).
